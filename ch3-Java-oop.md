...
| --- | --- | --- |
| static 변수 | 클래스 변수, 정적변수, 정적속성 | 메소드(스택) 영역(Method Area) |
| 인스턴스 변수 | 필드, 멤버 변수, 객체속성, 객체변수 | 힙(Heap) |
...
## 객체지향의 상속은 상위 클래스의 특성을 **재사용** 하는 것이다. 
## 객체 지향의 상속은 클래스의 특성을 **확장** 하는 것이다. 
...
---
# 🚫 1. 자바는 왜 “클래스 다중 상속”을 버렸나?

C++처럼 여러 클래스를 동시에 상속받으면 이런 문제가 발생합니다.
...
A의 메서드를 D가 호출하면 **B에서 온 건지 C에서 온 건지 모호**해짐.

이런 **모호성(ambiguity)**과 **복잡성** 때문에 유지보수가 매우 어려움.

👉 자바는 이 위험을 막기 위해 **클래스는 단일 상속만 허용**한다.

```java
class A {}
class B extends A {}   // OK
class C extends A {}   // OK
class D extends B, C {}  // ❌ 자바에서는 불가

```

---

# ✅ 2. 그렇다면 왜 인터페이스는 여러 개 구현 가능?

인터페이스는 **상태(필드)나 구현이 거의 없고 “추상적인 기능 목록”만 제공**하기 때문에

다중 상속의 위험 요소가 사라진다.

즉, 인터페이스는 이런 식의 “설계 계약서”이다:
...
두 인터페이스 모두 **메서드 시그니처만 제공할 뿐, 구현은 없으므로 충돌이 없다.**

---

# 🎯 3. 결론 (문장 해석)

**“자바는 인터페이스를 통해 다중 상속의 득을 취하고, 실은 과감히 버렸다.”**

이 말은 아래처럼 해석할 수 있다.

| 의미 | 설명 |
| --- | --- |
| **득(장점)** | 여러 클래스로부터 다양한 기능을 물려받는 다중 상속의 편리함을 인터페이스로 그대로 사용함 |
| **실(단점)** | 다중 상속 시 발생하는 복잡성, 모호성, 충돌 문제는 클래스 단일 상속만 허용함으로서 제거함 |

즉,

> 자바는 인터페이스로 “기능 다중 상속”을 가져왔고,
실제 구현을 복잡하게 만드는 “클래스 다중 상속”은 버렸다.
> 
...
- **클래스 상속:** 부모 집을 통째로 물려받는 것
    
    → 방 구조도, 빚도, 문제점도 모두 따라옴
    
- **인터페이스 구현:** "너는 영어 가능해야 하고, 운전도 할 줄 알아야 함" 이라는 규칙만 받는 것
    
    → 규칙만 받고, 실제 능력(구현)은 본인이 알아서 함
    

그래서 많은 규칙을 동시에 적용할 수 있다. (다중 인터페이스)
...
| **protected** | 같은 패키지 객체 + 상속받은 자식 객체, 다른 패키지 내 자식 클래스 | 부모의 protected 멤버는 자식 객체가 접근 가능 |
